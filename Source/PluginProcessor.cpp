...
// Фиксированный список частотных областей — ровно как в ТЗ, менять нельзя.
...
void EarTrainerAudioProcessor::startNewRound()
{
    ...
    currentGainDb    = (directionDist (rng) == 1) ? gainStepDb : -gainStepDb;
...
bool EarTrainerAudioProcessor::checkAnswer (int index)
{
    if (! roundActive)
        return false;

    const bool correct = (index == correctFreqIndex);
